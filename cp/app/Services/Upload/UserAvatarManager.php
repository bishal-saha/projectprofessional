...
class UserAvatarManager
{
    ...
    public function uploadAndCropAvatar(User $user, UploadedFile $file, array $cropPoints = null)
    {
        ...
        try {
            ...
            logger('Can not upload avatar '. $e->getMessage());
    ...
    {
       return $user->profile->avatar && ! str_contains($user->profile->avatar, ['http', 'https', 'gravatar']);
    }
    ...
    public function deleteAvatarIfUploaded(User $user)
    {
        ...
        $path = sprintf('%s/%s', $this->getDestinationDirectory(), $user->profile->avatar);
    ...
    private function cropAndResizeImage(File $avatarImage, array $points = null)
    {
        ...
        return $image->crop(self::AVATAR_WIDTH, self::AVATAR_HEIGHT)->save();
    }
}